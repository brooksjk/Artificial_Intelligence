...
def minimax(player, board, depth_limit):
    ...
    def max_value(player, board, depth):
        ...
        for action, new_board in get_child_boards(player, board):
            new_val, _ = value(board.PLAYER2, new_board, depth - 1)
            if new_val > v:
                v = new_val
                best_action = action
    ...
    def min_value(player, board, depth):
        ...
        best_action = None
        for action, new_board in get_child_boards(player, board):
            new_val, _ = value(board.PLAYER1, new_board, depth - 1)
            if new_val < v:
                v = new_val
                best_action = action
        return v, best_action

    _, placement = value(player, board, depth_limit)
    return placement

def alphabeta(player, board, depth_limit, alpha=-math.inf, beta=math.inf):
    """
    ...
    def max_value(player, board, depth, alpha, beta):
        ...
        best_action = None
        for action, new_board in get_child_boards(player, board):
            new_val, _ = value(board.PLAYER2, new_board, depth - 1, alpha, beta)
            if new_val > v:
                v = new_val
                best_action = action
            if v >= beta:
    ...
    def min_value(player, board, depth, alpha, beta):
        ...
        best_action = None
        for action, new_board in get_child_boards(player, board):
            new_val, _ = value(board.PLAYER1, new_board, depth - 1, alpha, beta)
            if new_val < v:
                v = new_val
                best_action = action
            if v <= alpha:
    ...
    _, placement = value(player, board, depth_limit, alpha, beta)
    return placement
